...
Phase-2 completion audit (2026-08-06); the mechanisms live in `src/db/seed.ts`
...
- **Version-gate what must run once; reconcile-at-launch what is idempotent.**
  ...
  re-run (icon reconciliation, config priming) runs at every launch instead —
...
- **Sync-safe by construction.** Migrations only ever ADD rows/columns or
  ...
  the formal resolution of the remaining skew question is tracked in the
  Phase-2 completion audit's ledger.
