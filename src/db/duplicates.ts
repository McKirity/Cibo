 ...
 * "Dark Souls III" beside the seeded one. **This is Data Doctor's
 * `entry-dedupe` check ([[Data Doctor (Reduced)]], Build step 13) arriving
 * early** — it lives in `src/db/` so step 13 inherits it rather than
 * re-implementing it, and the dev panel is a temporary door on it.
 *
 ...
 * a genuine identity.
 */
export const isSeedIdentity = (source: string | null, externalId: string | null): boolean =>
...
export type DuplicateCause =
   ...
   *  the seed data's fiction defeated both guards. Repair = the cover backfill,
   *  which rewrites seed ids to real ones. */
  | "seed-artifact"
...
/**
 * Scan one habit's entries for title-collisions. Title is the axis on purpose:
 * an identity collision alone cannot produce a visible twin (the hard guard
 * catches it), so what the user SEES as a duplicate is always two rows reading
 * the same.
 */
export const findDuplicates = async (habitKey: string): Promise<DuplicateReport> => {
  ...
  for (const s of sessions) {
    const k = s.entry_fk as string | null;
  ...
  for (const e of entries) {
    ...
    const row: DuplicateRow = {
      ...
      sessions: sessionCount.get(String(e.id)) ?? 0,
